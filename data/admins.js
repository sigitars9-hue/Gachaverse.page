// Auto-generated from form responses
export const admins = [
  {
    slug: "pacar-elysia",
    name: "Ryuu",
    img: "/admins/ryuu.jpg",
    role: "Founder",
    gender: "laki laki",
    skills: [
      "Communication",
      "Leadership",
      "Editing",
      "Programming"
    ],
    responsibility: "Mengelola semua akun Gachaverse di berbagai platfrom dan menentukan arah komunitas",
    joined: 2020,
    divisions: [
      "Founder",
      "Babel"
    ],
    languages: [
      "ID",
      "EN",
      "JP"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T12:52:02.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "hi3",
      "Genshin",
      "Pgr",
      "hsr"
    ],
    whatsapp: "6282123710880",
    channels: [
      {
        platform: "whatsapp",
        contact: "6282123710880",
        role: "Founder",
        responsibility: " Mengelola data komunitas",
        isPrimary: true
      }
    ]
  },
  {
    slug: "wira",
    name: "Wira",
    img: "/admins/wira.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Information",
      "Guide HI3"
    ],
    responsibility: "Saya bertanggung jawab atas kenyamanan dan keamanan di grup Anak Emas Kubis",
    joined: 2025,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WITA",
    status: "active",
    lastUpdated: "2025-10-06T12:52:02.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "hi3"
    ],
    whatsapp: "6282190622823",
    channels: [
      {
        platform: "whatsapp",
        contact: "6282190622823",
        role: "admin",
        responsibility: "Saya bertanggung jawab atas kenyamanan dan keamanan di grup Anak Emas Kubis",
        isPrimary: true
      }
    ]
  },

  {
    slug: "dav",
    name: "David",
    img: "/admins/dav.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Guide",
      "gameplay",
      "build reviews"
    ],
    responsibility: "Menangani member rusuh atau melanggar peraturan group dengan counter pendapat mereka",
    joined: 2024,
    divisions: [

    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T12:53:13.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "hsr",
      "zzz"
    ],
    whatsapp: "6287776161665",
    channels: [
      {
        platform: "whatsapp",
        contact: "6287776161665",
        role: "admin",
        responsibility: "Menangani member rusuh atau melanggar peraturan group dengan counter pendapat mereka",
        isPrimary: false
      }
    ]
  },

  {
    slug: "hoshilina",
    name: "Hoshilina",
    img: "/admins/hoshilina.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Lovestory writer"
    ],
    responsibility: "Mengawasi pemakaian ai",
    joined: 2025,
    divisions: [
      "AI"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T12:54:36.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "player-casual"
    ],
    whatsapp: "6282275392829",
    channels: [
      {
        platform: "whatsapp",
        contact: "6282275392829",
        role: "admin",
        responsibility: "Mengawasi pemakaian ai",
        isPrimary: true
      }
    ]
  },

  {
    slug: "haru",
    name: "Khairan",
    img: "/admins/haru.jpg",
    role: "Owner Gvstore",
    gender: "laki laki",
    skills: [
      "Communication",
      "Branding"
    ],
    responsibility: "Mengatur jalannya Joki & Menjaga kualitas dan kuantitas",
    joined: 2023,
    divisions: [
      "Owner Grup",
      "Gvs"
    ],
    languages: [
      "ID",
      "EN",
      "JP",
      "CN",
      "KR"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T12:55:34.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "mlbb",
      "gi",
      "hsr",
      "wuwa",
      "ak",
      "ba",
      "pgr",
      "hi3",
      "zzz",
      "al",
      "gfl"
    ],
    whatsapp: "6281259160766",
    channels: [
      {
        platform: "whatsapp",
        contact: "6281259160766",
        role: "owner",
        responsibility: "Job Matching",
        isPrimary: true
      }
    ]
  },

  {
    slug: "jz",
    name: "Jz",
    img: "/admins/jz.jpg",
    role: "Co-owner Group",
    gender: "laki laki",
    skills: [
      "Lore Analysis",
      "Game Infomation(Kurogames)."
    ],
    responsibility: "Membimbing member newbie",
    joined: 2024,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T12:55:48.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "pgr"
    ],
    whatsapp: "6285703033621",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285703033621",
        role: "admin",
        responsibility: "Membimbing member newbie",
        isPrimary: true
      }
    ]
  },

  {
    slug: "yasan",
    name: "yasan",
    img: "/admins/yasan.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Game Information",
      "Strategy",
      "Guide"
    ],
    responsibility: "Menjaga gachaverse menjadi tempat yang nyaman dan aman bagi setiap member komunitas.",
    joined: 2024,
    divisions: [
  
    ],
    languages: [
      "ID",
      "EN",
      "JP"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T12:57:56.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "mlbb",
      "hsr",
      "persona"
    ],
    whatsapp: "6282246047833",
    channels: [
      {
        platform: "whatsapp",
        contact: "6282246047833",
        role: "admin",
        responsibility: "Menjaga gachaverse menjadi tempat yang nyaman dan aman bagi setiap member nya untuk berinteraksi.",
        isPrimary: true
      }
    ]
  },

  {
    slug: "raidenz",
    name: "RaidenZ",
    img: "/admins/raidenz.jpg",
    role: "Co-owner Group",
    gender: "laki laki",
    skills: [
      "HI3 Guide",
      "Gameplay"
    ],
    responsibility: "Memastikan grup tetap aktif dan kondusif",
    joined: 2023,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T12:58:44.000Z",
    platforms: [
      "whatsapp",
      "discord"
    ],
    games: [
      "genshin",
      "hsr",
      "wuwa",
      "pgr",
      "hi3",
      "zzz"
    ],
    whatsapp: "6285155250004",
    discord: "https://discord.gg/RaidenZ#0927",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285155250004",
        role: "admin",
        responsibility: "Memastikan grup tetap aktif dan kondusif",
        isPrimary: true
      },
      {
        platform: "discord",
        url: "https://discord.gg/RaidenZ#0927",
        role: "admin",
        responsibility: "Memastikan grup tetap aktif dan kondusif",
        isPrimary: false
      }
    ]
  },

  {
    slug: "hyto",
    name: "Hyto",
    img: "/admins/hyto.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Guide",
      "game information"
    ],
    responsibility: "Mengawasi grup, menjaga ketertiban grup",
    joined: 2024,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T12:59:11.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "mlbb",
      "hsr",
      "pgr",
      "hi3"
    ],
    whatsapp: "6281382975970",
    channels: [
      {
        platform: "whatsapp",
        contact: "6281382975970",
        role: "admin",
        responsibility: "Mengawasi grup, menjaga ketertiban grup",
        isPrimary: true
      }
    ]
  },

  {
    slug: "lyana",
    name: "Lyana",
    img: "/admins/lyana.jpg",
    role: "Admin",
    gender: "perempuan",
    skills: [
      "Author",
      "Graphic Design"
    ],
    responsibility: "mengawasi grup kasual",
    joined: 2025,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T12:59:15.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "genshin",
      "hsr",
      "ak",
      "reverse1999"
    ],
    whatsapp: "6285214900240",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285214900240",
        role: "admin",
        responsibility: "mengawasi grup kasual",
        isPrimary: true
      }
    ]
  },

  {
    slug: "kamijou",
    name: "Kouta",
    img: "/admins/kamijou.jpg",
    role: "Owner Group",
    gender: "laki laki",
    skills: [
      "Game information"
    ],
    responsibility: "Menyambut member baru dan menertibkan grub, tanpa membuat suasana menjadi tegang dan member bisa santai.",
    joined: 2022,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T12:59:36.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "genshin"
    ],
    whatsapp: "6281235874049",
    channels: [
      {
        platform: "whatsapp",
        contact: "6281235874049",
        role: "admin",
        responsibility: "Menyambut member baru dan menertibkan grub, tanpa membuat suasana menjadi tegang dan member bisa santai.",
        isPrimary: false
      }
    ]
  },

  {
    slug: "xenozawa",
    name: "xenozawa",
    img: "/admins/xenozawa.jpg",
    role: "Owner Group",
    gender: "laki laki",
    skills: [
      "Game Guide",
      "Game Strategy"
    ],
    responsibility: "Owner group di beberapa group chat WA. Sesekali membantu guide untuk posting.",
    joined: 2021,
    divisions: [
      "Owner Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:00:07.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "hsr",
      "wuwa",
      "gbf",
      "r1999"
    ],
    whatsapp: "6282265091966",
    channels: [
      {
        platform: "whatsapp",
        contact: "6282265091966",
        role: "owner",
        responsibility: "Mengawasi dan mengatur grup",
        isPrimary: true
      }
    ]
  },

  {
    slug: "rizzmustdie",
    name: "Rizz",
    img: "/admins/rizz.jpg",
    role: "Head Admin",
    gender: "laki laki",
    skills: [
      "Guide",
      "Event Documentary",
      "Proofreader",
      "Leadership"
    ],
    responsibility: "Mengampu tugas sebagai Head Admin yang berperan sebagai penanggung jawab seluruh admin yang ada di Whatsapp Group, pembuat keputusan tahap awal, dan penanggung jawab temu sapa offline wilayah Jabodetabek",
    joined: 2021,
    divisions: [
      "Babel",
      "Owner Grup",
      "Admin Grup",
      "AI"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "UTC+7 (Jakarta)",
    status: "active",
    lastUpdated: "2025-10-06T13:01:55.000Z",
    platforms: [
      "whatsapp",
      "discord"
    ],
    games: [
      "wuwa",
      "ak",
      "ba",
      "pgr",
      "nikke"
    ],
    whatsapp: "6285710161768",
    discord: "https://discord.gg/rizzmustdie_",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285710161768",
        role: "admin",
        responsibility: "Head Admin",
        isPrimary: true
      },
      {
        platform: "discord",
        url: "https://discord.gg/rizzmustdie_",
        role: "moderator",
        responsibility: "Satpam",
        isPrimary: false
      }
    ]
  },

  {
    slug: "kanon",
    name: "Kanon",
    img: "/admins/kanon.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Guide",
      "Strategy",
      "Public Speaking"
    ],
    responsibility: "Memantau keadaan grup dan memastikan kondisi nya tetep aktif dan kondusif, serta memberikan update news dan guide sebisa dan sebaik mungkin ke member",
    joined: 2022,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN",
      "JP"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:02:04.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "hsr",
      "ba",
      "al",
      "gfl2"
    ],
    whatsapp: "62895345382926",
    channels: [
      {
        platform: "whatsapp",
        contact: "62895345382926",
        role: "admin",
        responsibility: "Memantau keadaan grup dan memastikan kondisi nya tetep aktif dan kondusif, serta memberikan update news dan guide sebisa dan sebaik mungkin ke member",
        isPrimary: true
      }
    ]
  },

  {
    slug: "fate",
    name: "Fate",
    img: "/admins/fate.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Guide",
      "information",
      "etc"
    ],
    responsibility: "Mengatur grup",
    joined: 2023,
    divisions: [
      "Leaks",
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN",
      "JP"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:02:25.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "mlbb",
      "genshin",
      "hsr",
      "wuwa",
      "zzz",
      "persona",
      "game-fps",
      "pc-game"
    ],
    whatsapp: "6285273874635",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285273874635",
        role: "admin",
        responsibility: "Mengatur grup",
        isPrimary: true
      }
    ]
  },

  {
    slug: "yharon",
    name: "Yharon",
    img: "/admins/yharon.jpg",
    role: "Co-owner Group",
    gender: "laki laki",
    skills: [
      "Strategizer",
      "Gameplay"
    ],
    responsibility: "Mengawasi traffic dan kelakuan anggota ACM.",
    joined: 2022,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "UTC+7 (Jakarta)",
    status: "active",
    lastUpdated: "2025-10-06T13:03:15.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "mlbb",
      "ak",
      "strinova"
    ],
    whatsapp: "6281291010707",
    channels: [
      {
        platform: "whatsapp",
        contact: "6281291010707",
        role: "admin",
        responsibility: "Mengawasi traffic dan kelakuan anggota ACM",
        isPrimary: true
      }
    ]
  },

  {
    slug: "llanfair",
    name: "Llanfair",
    img: "/admins/llanfair.jpg",
    role: "Co-owner Group",
    gender: "laki laki",
    skills: [
      "Game information",
      "Guide"
    ],
    responsibility: "Co-Owner grup WA Reverse:1999 dan admin post IG",
    joined: 2025,
    divisions: [
      "Posting",
      "Owner Grup"
    ],
    languages: [
      "ID",
      "EN",
      "AR"
    ],
    timezone: "UTC+7 (Jakarta)",
    status: "active",
    lastUpdated: "2025-10-06T13:06:13.000Z",
    platforms: [
      "whatsapp",
      "discord"
    ],
    games: [
      "r1999"
    ],
    whatsapp: "6285769067133",
    discord: "https://discord.com/users/705673146057097260",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285769067133",
        role: "owner",
        responsibility: "Co-Owner grup WA Reverse:1999 dan admin post IG",
        isPrimary: true
      },
      {
        platform: "discord",
        url: "https://discord.com/users/705673146057097260",
        role: "admin",
        responsibility: "Co-Owner grup WA Reverse:1999 dan admin post IG",
        isPrimary: false
      }
    ]
  },

  {
    slug: "gil",
    name: "Gil",
    img: "/admins/gil.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Game information",
      "Guide"
    ],
    responsibility: "Hsr",
    joined: 2024,
    divisions: [
      "Leaks",
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:08:21.000Z",
    platforms: [
      "whatsapp",
      "discord"
    ],
    games: [
      "genshin",
      "hsr",
      "zzz"
    ],
    whatsapp: "6285861751140",
    discord: "https://discord.gg/gil.rand",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285861751140",
        role: "admin",
        responsibility: "Hsr",
        isPrimary: false
      },
      {
        platform: "discord",
        url: "https://discord.gg/gil.rand",
        role: "admin",
        responsibility: "Hsr",
        isPrimary: false
      }
    ]
  },

  {
    slug: "altair",
    name: "Altair",
    img: "/admins/altair.jpg",
    role: "Owner Group",
    gender: "laki laki",
    skills: [
      "Guide",
      "recommendation (manhwa",
      "manga)"
    ],
    responsibility: "Menjaga ketertiban grub dan memantau setiap member untuk mematuhi segala peraturan yang ada.",
    joined: 2022,
    divisions: [
      "Posting",
      "Owner Grup"
    ],
    languages: [
      "ID"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:10:59.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "ak"
    ],
    whatsapp: "6285718072135",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285718072135",
        role: "owner",
        responsibility: "Menjaga ketertiban grub dan memantau setiap member untuk mematuhi segala peraturan yang ada.",
        isPrimary: true
      }
    ]
  },

  {
    slug: "raviann",
    name: "raviann",
    img: "/admins/raviann.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Game Guide"
    ],
    responsibility: "Menjadi pengarah yang baik dan ramah untuk para member PomPom",
    joined: 2024,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN",
      "Jawa"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:11:47.000Z",
    platforms: [
      "discord"
    ],
    games: [
      "mlbb",
      "hsr",
      "roblox"
    ],
    whatsapp: "#",
    discord: "https://discord.gg/rraviann.",
    channels: [
      {
        platform: "whatsapp",
        contact: "",
        role: "admin",
        responsibility: "Menjadi pengarah yang baik dan ramah untuk para member PomPom",
        isPrimary: true
      },
      {
        platform: "discord",
        url: "https://discord.gg/rraviann.",
        role: "admin",
        responsibility: "Menjadi pengarah yang baik dan ramah untuk para member PomPom",
        isPrimary: false
      }
    ]
  },

  {
    slug: "chloe",
    name: "Chloe",
    img: "/admins/chloe.jpg",
    role: "Admin",
    gender: "perempuan",
    skills: [
      "Gameplay",
      "Guide",
      "Game Information (LADS)"
    ],
    responsibility: "Admin grup.",
    joined: 2025,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:13:30.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "lads"
    ],
    whatsapp: "6285797027714",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285797027714",
        role: "admin",
        responsibility: "6285797027714",
        isPrimary: true
      }
    ]
  },

  {
    slug: "shizuu",
    name: "Shizuu",
    img: "/admins/shizuu.jpg",
    role: "Owner Group",
    gender: "laki laki",
    skills: [
      "Gameplay",
      "Guide",
      "Server Moderator"
    ],
    responsibility: "Mengawasi grup Eridu, bertaggung jawab atas Grup Minecraft dan Shooter, dan Mengawasi traffic Discord",
    joined: 2023,
    divisions: [
      "Leaks",
      "Owner Grup",
      "Admin Grup",
      "Discord"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "UTC+7 (Jakarta)",
    status: "active",
    lastUpdated: "2025-10-06T13:15:34.000Z",
    platforms: [
      "whatsapp",
      "discord"
    ],
    games: [
      "hsr",
      "zzz",
      "roblox",
      "duet-nigh-abyss",
      "valorant",
      "mongol-star-dive",
      "uma-musume-jp",
      "osu"
    ],
    whatsapp: "6281383216705",
    discord: "https://discord.gg/yMQKQpkwss",
    channels: [
      {
        platform: "whatsapp",
        contact: "6281383216705",
        role: "admin",
        responsibility: "Mengawasi grup Eridu, bertaggung jawab atas Grup Minecraft dan Shooter, dan Mengawasi traffic Discord",
        isPrimary: false
      },
      {
        platform: "discord",
        url: "https://discord.gg/yMQKQpkwss",
        role: "moderator",
        responsibility: "Mengawasi Traffic",
        isPrimary: true
      }
    ]
  },

  {
    slug: "syaa",
    name: "Syaa",
    img: "/admins/syaa.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "mendemokan gameplay",
      "meta dasar",
      "dan tips mekanik"
    ],
    responsibility: "mengawasi member",
    joined: 2024,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:17:07.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "mlbb",
      "genshin",
      "hsr",
      "wuwa",
      "zzz"
    ],
    whatsapp: "6285882821862",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285882821862",
        role: "admin",
        responsibility: "mengawasi member",
        isPrimary: true
      }
    ]
  },

  {
    slug: "rui",
    name: "Rui",
    img: "/admins/rui.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Guide",
      "Tactician",
      "Design Graphic"
    ],
    responsibility: "Berperan dalam mengawasi keadaan Grup Perusahaan Hitam dan memastikan traffic ramai, dan menegakkan Hukum yang berlaku dalam GCV",
    joined: 2024,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:17:12.000Z",
    platforms: [
      "discord"
    ],
    games: [
      "mlbb",
      "wuwa",
      "ak",
      "pgr"
    ],
    whatsapp: "#",
    discord: "https://discord.com/channels/@Kazerui",
    channels: [
      {
        platform: "whatsapp",
        contact: "",
        role: "admin",
        responsibility: "Mengawasi dan Menegakkan Hukum di Grup Perusahaan Hitam",
        isPrimary: true
      },
      {
        platform: "discord",
        url: "https://discord.com/channels/@Kazerui",
        role: "admin",
        responsibility: "#",
        isPrimary: false
      }
    ]
  },

  {
    slug: "kaito",
    name: "Kaito",
    img: "/admins/kaito.jpg",
    role: "Owner Group",
    gender: "laki laki",
    skills: [
      "Guide",
      "Sticker Maker",
      "Strategy",
      "Artist"
    ],
    responsibility: "Owner Dante",
    joined: 2022,
    divisions: [
      "Leaks",
      "Owner Grup",
      "Admin Grup",
      "Gvs"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:20:24.000Z",
    games: [
      "hsr",
      "wuwa",
      "ak",
      "pgr",
      "hi3",
      "zzz",
      "al",
      "gfl2"
    ],
    whatsapp: "#",
    channels: [
      {
        platform: "whatsapp",
        contact: "",
        role: "owner",
        responsibility: "Owner Dante",
        isPrimary: true
      }
    ]
  },

  {
    slug: "yuki",
    name: "Yuki",
    img: "/admins/yuki.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Guide character hsr",
      "anime information"
    ],
    responsibility: "Mengawasi grup casual",
    joined: 2025,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:22:55.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "mlbb",
      "hsr"
    ],
    whatsapp: "6283820615118",
    channels: [
      {
        platform: "whatsapp",
        contact: "6283820615118",
        role: "admin",
        responsibility: "Mengawasi grup casual",
        isPrimary: true
      }
    ]
  },

  {
    slug: "apexxa",
    name: "APexxa",
    img: "/admins/pexxa.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Strategy",
      "tester Yura analysis",
      "beginners programming"
    ],
    responsibility: "memantau grub mahasiswa,menjadi beta tester Yura AI Dan membantu admin lain yang yang membutuhkan",
    joined: 2024,
    divisions: [
      "Admin Grup",
      "AI",
   
    ],
    languages: [
      "ID"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:24:44.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "mlbb",
      "hsr",
      "clash-of-clan",
      "clash-royale",
      "delta-force",
      "pubgm"
    ],
    whatsapp: "6282177517990",
    channels: [
      {
        platform: "whatsapp",
        contact: "6282177517990",
        role: "admin",
        responsibility: "Mengawasi grup casual",
        isPrimary: true
      }
    ]
  },

  {
    slug: "izuru",
    name: "Izuru",
    img: "/admins/izu.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Video Game Guide",
      "Lore Analysis"
    ],
    responsibility: "Memantau Grup TCG agar orang orang bisa bertukar strategi tentang Card Game",
    joined: 2024,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN",
      "JP"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:25:22.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "genshin",
      "hsr",
      "ak",
      "ba",
      "pgr",
      "hi3",
      "zzz",
      "persona"
    ],
    whatsapp: "6285883467883",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285883467883",
        role: "admin",
        responsibility: "Memantau Grup TCG agar orang orang bisa bertukar strategi tentang Card Game",
        isPrimary: true
      }
    ]
  },

  {
    slug: "fay",
    name: "Fay",
    img: "/admins/fay.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Analysis",
      "strategi",
      "Guide",
      "fast learner",
      "rythme"
    ],
    responsibility: "Divisi UmaMusume",
    joined: 2025,
    divisions: [
      "Posting",
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:25:47.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "uma"
    ],
    whatsapp: "6282114911824",
    channels: [
      {
        platform: "whatsapp",
        contact: "6282114911824",
        role: "admin",
        responsibility: "Divisi UmaMusume",
        isPrimary: true
      }
    ]
  },

  {
    slug: "ceezu",
    name: "Ceezu",
    img: "/admins/ceezu.jpg",
    role: "Sekretaris",
    gender: "laki laki",
    skills: [
      "Video Editor",
      "Content Creator",
      "Designer",
      "Hoyoverse Guide (GI",
      "HSR",
      "ZZZ)"
    ],
    responsibility: "- Bertanggung jawab atas manajemen operasional komunitas seperti Mendesain Profile Komunitas/Grup, Pembersihan member di Komunitas jika penuh dan memberikan sanksi kepada member yang melanggar aturan\n- Bertanggung jawab atas Konten Posting di Instagram dan Tiktok Gachaverse seperti membuat konten dengan Quality tinggi dll.",
    joined: 2024,
    divisions: [
      "Babel",
      "Posting",
      "Owner Grup",
      "Admin Grup",
      "Gvs"
    ],
    languages: [
      "ID",
      "EN",
      "Jawa"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:26:02.000Z",
    platforms: [
      "whatsapp",
      "discord"
    ],
    games: [
      "mlbb",
      "genshin",
      "hsr",
      "zzz",
      "al",
      "strinova",
      "delta-force",
      "gtav",
      "valorant"
    ],
    whatsapp: "6281217131696",
    discord: "https://discord.com/channels/@ceezumv",
    channels: [
      {
        platform: "whatsapp",
        contact: "6281217131696",
        role: "editor",
        responsibility: "Bertanggung jawab atas manajemen beberapa grup di komunitas Gachaverse (ZZZ, Kreator, Casual, Shooter dll.)",
        isPrimary: true
      },
      {
        platform: "discord",
        url: "https://discord.com/channels/@ceezumv",
        role: "admin",
        responsibility: "- Bertanggung jawab atas manajemen operasional komunitas seperti Mendesain Profile Komunitas/Grup, Pembersihan member di Komunitas jika penuh dan memberikan sanksi kepada member yang melanggar aturan\n- Bertanggung jawab atas Konten Posting di Instagram dan Tiktok Gachaverse seperti membuat konten dengan Quality tinggi dll.",
        isPrimary: false
      }
    ]
  },

  {
    slug: "datensh",
    name: "DaTensh",
    img: "/admins/datensh.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Typesetter",
      "Manga",
      "Game Information"
    ],
    responsibility: "mengawasi traffic discord",
    joined: 2023,
    divisions: [
      "Leaks",
      "Owner Grup",
      "Admin Grup",
      "Discord"
    ],
    languages: [
      "ID",
      "EN",
      "jp rada rada"
    ],
    timezone: "UTC+7 (Jakarta)",
    status: "active",
    lastUpdated: "2025-10-06T13:26:45.000Z",
    platforms: [
      "whatsapp",
      "discord"
    ],
    games: [
      "hsr",
      "ak",
      "ba",
      "pgr",
      "hi3",
      "reverse",
      "1999"
    ],
    whatsapp: "6295326674483",
    discord: "https://discord.gg/ChWkrccZpu",
    channels: [
      {
        platform: "whatsapp",
        contact: "6295326674483",
        role: "admin",
        responsibility: "mengawasi traffic discord",
        isPrimary: true
      },
      {
        platform: "discord",
        url: "https://discord.gg/ChWkrccZpu",
        role: "admin",
        responsibility: "mengawasi traffic discord, mengawasi moderator yang lain",
        isPrimary: true
      }
    ]
  },

  {
    slug: "reteg",
    name: "Reteg",
    img: "/admins/reteg.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Theorycrafting",
      "Guide"
    ],
    responsibility: "Memberikan guide dan theorycraft untuk resonator2 yang ada di WuWa dan menjaga kekondusivitas grup",
    joined: 2022,
    divisions: [
      "Posting",
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN",
      "Jawa"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:29:26.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "mlbb",
      "genshin",
      "wuwa",
      "fps"
    ],
    whatsapp: "628129780493",
    channels: [
      {
        platform: "whatsapp",
        contact: "628129780493",
        role: "admin",
        responsibility: "Memberikan guide dan theorycraft untuk resonator2 yang ada di WuWa dan menjaga kekondusivitas grup",
        isPrimary: true
      }
    ]
  },

  {
    slug: "rei",
    name: "Rei",
    img: "/admins/rei.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Guide",
      "Game Information"
    ],
    responsibility: "membantu member",
    joined: 2025,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:30:06.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "ak",
      "ba"
    ],
    whatsapp: "6282234123314",
    channels: [
      {
        platform: "whatsapp",
        contact: "6282234123314",
        role: "admin",
        responsibility: "Sharing info, guide & news",
        isPrimary: true
      }
    ]
  },

  {
    slug: "st-anth",
    name: "St.Anth",
    img: "/admins/st-anth.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Game information",
      "Analisis data (Secara statistik)"
    ],
    responsibility: "Menjaga ketertiban dan suasana yang kondusif untuk melakukan pembicaraan dalam grup",
    joined: 2024,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:30:46.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "hsr",
      "wuwa"
    ],
    whatsapp: "6287883760483",
    channels: [
      {
        platform: "whatsapp",
        contact: "6287883760483",
        role: "admin",
        responsibility: "Menjaga ketertiban grup",
        isPrimary: true
      }
    ]
  },

  {
    slug: "kurokaizaru",
    name: "Kurokaizaru",
    img: "/admins/kurokaizaru.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Srategy",
      "guide",
      "analyst"
    ],
    responsibility: "Arknight character/stage guide",
    joined: 2025,
    divisions: [
      "Posting"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:34:57.000Z",
    platforms: [
      "discord"
    ],
    games: [
      "hsr",
      "ak",
      "code-vein"
    ],
    whatsapp: "#",
    discord: "https://discord.gg/Kurokaizaru7#5613",
    channels: [
      {
        platform: "whatsapp",
        contact: "",
        role: "admin",
        responsibility: "Arknight character/stage guide",
        isPrimary: true
      },
      {
        platform: "discord",
        url: "https://discord.gg/Kurokaizaru7#5613",
        role: "admin",
        responsibility: "Arknight character/stage guide",
        isPrimary: false
      }
    ]
  },

  {
    slug: "mimi",
    name: "Mimi",
    img: "/admins/mimi.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Story Reader",
      "Information",
      "Theory",
      "Research",
      "Humble",
      "Sobat Gurun"
    ],
    responsibility: "Mengolah cerita dan informasi lewat riset untuk data yang valid; ramah tapi tegas saat diperlukan",
    joined: 2023,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "UTC+7 (Jakarta)",
    status: "active",
    lastUpdated: "2025-10-06T13:36:00.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "ak",
      "hi3",
      "al"
    ],
    whatsapp: "6287784093452",
    channels: [
      {
        platform: "whatsapp",
        contact: "6287784093452",
        role: "admin",
        responsibility: "Information, Research, Theory, Data",
        isPrimary: true
      }
    ]
  },

  {
    slug: "negep",
    name: "NegeP",
    img: "/admins/negep.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Guide (Rhythm)",
      "Game Information (Rhythm)",
      "Leluhur Gameplay (Rhythm) Terutama OSU"
    ],
    responsibility: "Menyambut dan mengawasi member, menjawab pertanyaan member",
    joined: 2025,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:37:12.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "ak",
      "zzz",
      "al",
      "bandori",
      "pjsk",
      "d4dj",
      "wds",
      "arcaea",
      "phigros",
      "ellia",
      "cytus",
      "takumi",
      "osu",
      "muse-dash",
      "maimai",
      "chunithm"
    ],
    whatsapp: "6287816945815",
    channels: [
      {
        platform: "whatsapp",
        contact: "6287816945815",
        role: "admin",
        responsibility: "Menyambut dan mengawasi member, menjawab pertanyaan member, eksekusi member yang melanggar peraturan grup",
        isPrimary: true
      }
    ]
  },

  {
    slug: "asuka",
    name: "Asuka",
    img: "/admins/asuka.jpg",
    role: "Best Creator",
    gender: "perempuan",
    skills: [
      "Video editing",
      "art",
      "editing"
    ],
    responsibility: "Posting & mengawasi grup",
    joined: 2023,
    divisions: [
      "Babel",
      "Posting",
      "Leaks",
      "Owner Grup",
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN",
      "JP"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:37:59.000Z",
    platforms: [
      "whatsapp",
      "discord"
    ],
    games: [
      "hsr",
      "ak",
      "hi3",
      "fellow-moon",
      "reverse-1999",
      "fgo",
      "lads"
    ],
    whatsapp: "6285261809904",
    discord: "https://--",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285261809904",
        role: "admin",
        responsibility: "Posting & mengawasi grup",
        isPrimary: true
      },
      {
        platform: "discord",
        url: "https://--",
        role: "admin",
        responsibility: "Posting & mengawasi grup",
        isPrimary: false
      }
    ]
  },

  {
    slug: "seira",
    name: "Seira",
    img: "/admins/seira.jpg",
    role: "Admin",
    gender: "perempuan",
    skills: [
      "editing",
      "drawings",
      "graphic designer"
    ],
    responsibility: "informasi LaDS",
    joined: 2025,
    divisions: [
      "Posting"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:38:26.000Z",
    platforms: [
      "discord"
    ],
    games: [
      "genshin",
      "hsr",
      "wuwa",
      "roblox",
      "lads",
      "haikyuu"
    ],
    whatsapp: "#",
    discord: "https://discord.gg/seii1_",
    channels: [
      {
        platform: "whatsapp",
        contact: "",
        role: "admin",
        responsibility: "informasi LaDS",
        isPrimary: false
      },
      {
        platform: "discord",
        url: "https://discord.gg/seii1_",
        role: "admin",
        responsibility: "informasi LaDS",
        isPrimary: false
      }
    ]
  },

  {
    slug: "soda",
    name: "Soda",
    img: "/admins/soda.jpg",
    role: "Co-owner group",
    gender: "laki laki",
    skills: [
      "Gameplay",
      "Guide",
      "Posting"
    ],
    responsibility: "Content Creator untuk Gachaverse dan Sebagai Co-Owner Anak Pungut Eridu.",
    joined: 2025,
    divisions: [
      "Posting",
      "Admin Grup",
      "Discord"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:38:35.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "mlbb",
      "zzz",
      "persona",
      "umamusume"
    ],
    whatsapp: "6285174253631",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285174253631",
        role: "editor",
        responsibility: "Funfact serta guide seputar ZZZ dan Umamusume",
        isPrimary: true
      }
    ]
  },
  {
    slug: "jul",
    name: "Jul",
    img: "/admins/jul.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Guide"
    ],
    responsibility: "Bertanggung jawab sebagai admin gc casual",
    joined: 2023,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID"
    ],
    timezone: "UTC+7 (Jakarta)",
    status: "active",
    lastUpdated: "2025-10-06T13:54:39.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "wuwa",
      "valorant",
      "war-thunder"
    ],
    whatsapp: "6285156119507",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285156119507",
        role: "admin",
        responsibility: "Bertanggung jawab sebagai admin gc casual",
        isPrimary: true
      }
    ]
  },

  {
    slug: "wldanee",
    name: "Wldannn",
    img: "/admins/wldannn.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Game Guide",
      "All about moba game",
      "Leak or Patch information"
    ],
    responsibility: "Membantu member",
    joined: 2024,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T13:58:17.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "mlbb",
      "apapun-itu-asal-genrenya-moba"
    ],
    whatsapp: "6285748125249",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285748125249",
        role: "admin",
        responsibility: "Membantu member",
        isPrimary: true
      }
    ]
  },

  {
    slug: "nandahh",
    name: "Nandahh",
    img: "/admins/nandahh.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Designer"
    ],
    responsibility: "Membuat design template feed, logo dan  design poster penjualan gvstore. Dan juga memantau grup pojok kreator dan roblox.",
    joined: 2023,
    divisions: [
      "Admin Grup",
      "Discord"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WITA",
    status: "active",
    lastUpdated: "2025-10-06T13:59:08.000Z",
    platforms: [
      "whatsapp",
      "discord"
    ],
    games: [
      "hsr",
      "roblox",
      "umamusume"
    ],
    whatsapp: "6281353857668",
    discord: "https://discord.gg/itznandahh",
    channels: [
      {
        platform: "whatsapp",
        contact: "6281353857668",
        role: "admin",
        responsibility: "Membuat design template feed, logo dan  design poster penjualan gvstore. Dan juga memantau grup pojok kreator dan roblox.",
        isPrimary: true
      },
      {
        platform: "discord",
        url: "https://discord.gg/itznandahh",
        role: "admin",
        responsibility: "Membuat design template feed, logo dan  design poster penjualan gvstore. Dan juga memantau grup pojok kreator dan roblox.",
        isPrimary: true
      }
    ]
  },

  {
    slug: "goro",
    name: "Goro",
    img: "/admins/goro.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Guide",
      "strategy"
    ],
    responsibility: "Menjaga grup",
    joined: 2023,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WITA",
    status: "active",
    lastUpdated: "2025-10-06T14:00:52.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "mlbb",
      "hsr",
      "fate-go"
    ],
    whatsapp: "6289520174379",
    channels: [
      {
        platform: "whatsapp",
        contact: "6289520174379",
        role: "admin",
        responsibility: "Menjaga grup",
        isPrimary: true
      }
    ]
  },

  {
    slug: "pal",
    name: "Pal",
    img: "/admins/pal.jpg",
    role: "moderator",
    gender: "laki laki",
    skills: [
      "Strategy",
      "Guide for game"
    ],
    responsibility: "Menjaga keamanan dan ketentraman komunitas dalam discord",
    joined: 2025,
    divisions: [
      "Discord"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T14:03:10.000Z",
    platforms: [
      "whatsapp",
      "discord"
    ],
    games: [
      "hsr",
      "roblox"
    ],
    whatsapp: "6287848111816",
    discord: "https://discord.gg/vUxQhQ96",
    channels: [
      {
        platform: "whatsapp",
        contact: "6287848111816",
        role: "admin",
        responsibility: "Menjaga keamanan dan ketentraman komunitas dalam discord",
        isPrimary: false
      },
      {
        platform: "discord",
        url: "https://discord.gg/vUxQhQ96",
        role: "admin",
        responsibility: "Menjaga keamanan dan ketentraman komunitas dalam discord",
        isPrimary: false
      }
    ]
  },

  {
    slug: "nishikawa",
    name: "Vin",
    img: "/admins/nishikawa.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Guide",
      "Game Information About Aether Gazer"
    ],
    responsibility: "Berusaha fast Respon ketika di tag perihal sesuatu namun melihat kondisi dulu.",
    joined: 2023,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T14:03:59.000Z",
    platforms: [
      "whatsapp",
      "discord"
    ],
    games: [
      "aether-gazer"
    ],
    whatsapp: "6285649155051",
    discord: "https://discord.gg/xxxxx",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285649155051",
        role: "admin",
        responsibility: "Berusaha fast Respon ketika di tag perihal sesuatu namun melihat kondisi dulu.",
        isPrimary: true
      },
      {
        platform: "discord",
        url: "https://discord.gg/xxxxx",
        role: "admin",
        responsibility: "Berusaha fast Respon ketika di tag perihal sesuatu namun melihat kondisi dulu.",
        isPrimary: false
      }
    ]
  },

  {
    slug: "ark",
    name: "Ark",
    img: "/admins/ark.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Guide"
    ],
    responsibility: "Mantau grup",
    joined: 2024,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T14:07:54.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "ak",
      "pgr",
      "aether-gazer",
      "gbf",
      "shadowverse"
    ],
    whatsapp: "6289601578562",
    channels: [
      {
        platform: "whatsapp",
        contact: "6289601578562",
        role: "admin",
        responsibility: "Mantau grup",
        isPrimary: true
      }
    ]
  },

  {
    slug: "asa",
    name: "Asa",
    img: "/admins/asa.jpg",
    role: "Moderator",
    gender: "laki laki",
    skills: [
      "moderating bot",
      "channel"
    ],
    responsibility: "Mengawasi serta mengatur jalannya server discord tetap kondusif. Serta melayani pembuatan role untuk benefit para booster server.",
    joined: 2024,
    divisions: [
      "Discord"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "UTC+7 (Jakarta)",
    status: "active",
    lastUpdated: "2025-10-06T14:15:29.000Z",
    platforms: [
      "discord"
    ],
    games: [
      "hsr",
      "wuwa",
      "ak",
      "hi3"
    ],
    whatsapp: "#",
    discord: "https://discord.gg/urdnK2RU",
    channels: [
      {
        platform: "whatsapp",
        contact: "",
        role: "admin",
        responsibility: "Mengawasi serta mengatur jalannya server discord tetap kondusif. Serta melayani pembuatan role untuk benefit para booster server.",
        isPrimary: false
      },
      {
        platform: "discord",
        url: "https://discord.gg/urdnK2RU",
        role: "moderator",
        responsibility: "Moderating bot, server and roles",
        isPrimary: true
      }
    ]
  },

  {
    slug: "tia",
    name: "Tia",
    img: "/admins/tia.jpg",
    role: "Co-owner Group",
    gender: "laki laki",
    skills: [
      "Guide",
      "Rhythm"
    ],
    responsibility: "Memantau keadaan pada grup yang dipegang, serta memberikan panduan pada game Blue Archive dan beberapa Rhythm game",
    joined: 2023,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T14:20:55.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "ak",
      "ba",
      "pgr",
      "pjsk",
      "bandori",
      "maimai"
    ],
    whatsapp: "6289661457400",
    channels: [
      {
        platform: "whatsapp",
        contact: "6289661457400",
        role: "admin",
        responsibility: "Guide seputar Blue Archive dan beberapa Rhythm game",
        isPrimary: true
      }
    ]
  },

  {
    slug: "imy",
    name: "imy",
    img: "/admins/imy.jpg",
    role: "Sekretaris",
    gender: "perempuan",
    skills: [
      "Analyst",
      "Event Organizer",
      "Junior Programmer",
      "Top Global edith"
    ],
    responsibility: "Mengatur Pencatatan GVStore, Mengatur Admin Grup Anak Coklat Moonton, Prompter Yura AI, Worker Joki",
    joined: 2022,
    divisions: [
      "Owner Grup",
      "Admin Grup",
      "Discord",
      "AI",
      "Gvs"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T14:35:18.000Z",
    platforms: [
      "discord"
    ],
    games: [
      "mlbb",
      "genshin",
      "hsr",
      "wuwa",
      "hi3",
      "zzz"
    ],
    whatsapp: "#",
    discord: "https://discord.gg/VeRWjwx2",
    channels: [
      {
        platform: "whatsapp",
        contact: "",
        role: "admin",
        responsibility: "Mengatur Pencatatan GVStore, Mengatur Admin Grup Anak Coklat Moonton, Prompter Yura AI, Worker Joki",
        isPrimary: true
      },
      {
        platform: "discord",
        url: "https://discord.gg/VeRWjwx2",
        role: "admin",
        responsibility: "Mengatur Pencatatan GVStore, Mengatur Admin Grup Anak Coklat Moonton, Prompter Yura AI, Worker Joki",
        isPrimary: true
      }
    ]
  },

  {
    slug: "rovalian",
    name: "Rovalian",
    img: "/admins/rovalian.jpg",
    role: "Owner Group",
    gender: "laki laki",
    skills: [
      "Japanese Language"
    ],
    responsibility: "Menjaga keamanan grup, meramaikan grup, dan membantu member",
    joined: 2023,
    divisions: [
      "Posting",
      "Translator",
      "Owner Grup"
    ],
    languages: [
      "ID",
      "JP"
    ],
    timezone: "UTC+7 (Jakarta)",
    status: "active",
    lastUpdated: "2025-10-06T14:38:17.000Z",
    platforms: [
      "whatsapp"
    ],
    whatsapp: "6285895981335",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285895981335",
        role: "owner",
        responsibility: "Menjaga keamanan grup, meramaikan grup, dan membantu member",
        isPrimary: true
      }
    ]
  },

  {
    slug: "hikarin",
    name: "HikarinV37",
    img: "/admins/hikarin.jpg",
    role: "Owner Group",
    gender: "laki laki",
    skills: [
      "Guide",
      "Game Information"
    ],
    responsibility: "Penanggung jawab divisi grup mengenai Kurogames",
    joined: 2024,
    divisions: [
      "Posting",
      "Owner Grup"
    ],
    languages: [
      "ID",
      "EN",
      "JP"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T14:39:13.000Z",
    games: [
      "wuwa",
      "pgr"
    ],
    whatsapp: "#",
    channels: [
      {
        platform: "whatsapp",
        contact: "",
        role: "owner",
        responsibility: "6285892782249",
        isPrimary: true
      }
    ]
  },

  {
    slug: "zack",
    name: "Zack",
    img: "/admins/zack.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Typesetter",
      "Game Information",
      "Gaming",
      "Minecraft Command Builder"
    ],
    responsibility: "Penegak aturan GV di grup WA; builder command block Minecraft.",
    joined: 2023,
    divisions: [
      "Translator",
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN",
      "AR"
    ],
    timezone: "Lainnya",
    status: "active",
    lastUpdated: "2025-10-06T14:53:48.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "hsr",
      "wuwa",
      "hi3",
      "valorant",
      "minecraft",
      "uma",
      "reverse-1999"
    ],
    whatsapp: "201505839660",
    channels: [
      {
        platform: "whatsapp",
        contact: "201505839660",
        role: "admin",
        responsibility: "Moderasi & bantuan teknis.",
        isPrimary: true
      }
    ]
  },

  {
    slug: "blo",
    name: "Tablo",
    img: "/admins/blo.jpg",
    role: "Moderator",
    gender: "laki laki",
    skills: [
      "Strategy",
      "Business",
      "Psychology",
      "Public Relation"
    ],
    responsibility: "Memantau Discord",
    joined: 2024,
    divisions: [
      "Discord"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T15:12:15.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "mlbb",
      "wuwa",
      "ba",
      "al"
    ],
    whatsapp: "6287769176886",
    channels: [
      {
        platform: "whatsapp",
        contact: "6287769176886",
        role: "admin",
        responsibility: "Memantau Discord",
        isPrimary: false
      }
    ]
  },

  {
    slug: "faiz",
    name: "Faiz",
    img: "/admins/faiz.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Yura trainer",
      "strategy",
      "comicks recommendations"
    ],
    responsibility: "AI training",
    joined: 2025,
    divisions: [
      "Admin Grup",
      "AI"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WITA",
    status: "active",
    lastUpdated: "2025-10-06T15:13:23.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "hsr",
      "ak",
      "ba",
      "zzz",
      "limbus-company"
    ],
    whatsapp: "62895336677729",
    channels: [
      {
        platform: "whatsapp",
        contact: "62895336677729",
        role: "admin",
        responsibility: "AI training",
        isPrimary: true
      }
    ]
  },

  {
    slug: "dhani-ali",
    name: "Dhani Ali",
    img: "/admins/dhani-ali.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Game information"
    ],
    responsibility: "Mengawasi grup penjaga anomali (guardian tales)",
    joined: 2024,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID"
    ],
    timezone: "WIT",
    status: "active",
    lastUpdated: "2025-10-06T15:21:43.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "ba",
      "pgr",
      "gt",
      "gbf"
    ],
    whatsapp: "6281221529216",
    channels: [
      {
        platform: "whatsapp",
        contact: "6281221529216",
        role: "admin",
        responsibility: "Mengawasi grup penjaga anomali (guardian tales)",
        isPrimary: true
      }
    ]
  },

  {
    slug: "kazari",
    name: "Kazari",
    img: "/admins/kazari.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Game Information",
      "Story Telling",
      "Design",
      "Video Editing"
    ],
    responsibility: "Membuat postingan tentang game project Sekai baik Informasi, Event, Story (Kedepannya).",
    joined: 2025,
    divisions: [
      "Posting"
    ],
    languages: [
      "ID",
      "EN",
      "JP",
      "Arabic",
      "Malay"
    ],
    timezone: "UTC+7 (Jakarta)",
    status: "active",
    lastUpdated: "2025-10-06T15:27:25.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "genshin",
      "pjsk"
    ],
    whatsapp: "6285171076225",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285171076225",
        role: "admin",
        responsibility: "Posting",
        isPrimary: true
      }
    ]
  },

  {
    slug: "kalandra",
    name: "Kalandra",
    img: "/admins/kalandra.jpg",
    role: "Admin",
    gender: "perempuan",
    skills: [
      "Video editing",
      "graphic designer"
    ],
    responsibility: "Mengatur ketertiban di dalam grup",
    joined: 2025,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WITA",
    status: "active",
    lastUpdated: "2025-10-06T15:50:12.000Z",
    games: [
      "wuwa",
      "zzz"
    ],
    whatsapp: "#",
    channels: [
      {
        platform: "whatsapp",
        contact: "",
        role: "admin",
        responsibility: "Mengatur ketertiban di dalam grup",
        isPrimary: true
      }
    ]
  },

  {
    slug: "zuee",
    name: "Chizue",
    img: "/admins/zuee.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Graphic Design",
      "Video Edit"
    ],
    responsibility: "MenTransliterasi Karya sastra Jepang ke bahasa Indonesia",
    joined: 2023,
    divisions: [
      "Translator"
    ],
    languages: [
      "ID",
      "EN",
      "JP"
    ],
    timezone: "WIB",
    status: "inactive",
    lastUpdated: "2025-10-06T16:03:31.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "mlbb",
      "codm",
      "pjsk",
      "bandori"
    ],
    whatsapp: "62895341667044",
    channels: [
      {
        platform: "whatsapp",
        contact: "62895341667044",
        role: "editor",
        responsibility: "MenTransliterasi Karya sastra Jepang ke bahasa Indonesia",
        isPrimary: true
      }
    ]
  },

  {
    slug: "sir-if-pratama-xii",
    name: "Sir I.F Pratama XII",
    img: "/admins/sir-if-pratama-xii.jpg",
    role: "Co-owner Group",
    gender: "laki laki",
    skills: [
      "Guide",
      "Theorycrafting",
      "Leaks",
      "Gameplay",
      "Strategy"
    ],
    responsibility: "Meluruskan dan menuntun player Genshin ke jalan meta yang benar",
    joined: 2023,
    divisions: [
      "Leaks",
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN",
      "Sunda \ud83d\udd25"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T16:11:54.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "genshin",
      "wuwa",
      "minecraft"
    ],
    whatsapp: "6283157615818",
    channels: [
      {
        platform: "whatsapp",
        contact: "6283157615818",
        role: "admin",
        responsibility: "Menjawab pertanyaan, memberikan guide, dan record rotasi untuk referensi",
        isPrimary: true
      }
    ]
  },

  {
    slug: "mz",
    name: "Mz",
    img: "/admins/mz.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Strategy",
      "guide",
      "build",
      "gameplay"
    ],
    responsibility: "Keep active as user, tetap membuatnya di bawah kendali",
    joined: 2025,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T16:21:42.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "hsr",
      "ak",
      "ba",
      "hi3",
      "zzz",
      "limbus-company",
      "umamusume"
    ],
    whatsapp: "62895806428239",
    channels: [
      {
        platform: "whatsapp",
        contact: "62895806428239",
        role: "admin",
        responsibility: "As normal admin, menjaga ketertiban dalam grup yang di jaga",
        isPrimary: true
      }
    ]
  },

  {
    slug: "raven",
    name: "R",
    img: "/admins/r.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Guide (PGR",
      "Wuwa)",
      "Information (PGR"
    ],
    responsibility: "Admin group Perusahaan hitam | Kurogames",
    joined: 2023,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T16:28:53.000Z",
    platforms: [
      "discord"
    ],
    games: [
      "wuwa",
      "pgr"
    ],
    whatsapp: "#",
    discord: "https://discord.gg/users/458699026880266280",
    channels: [
      {
        platform: "whatsapp",
        contact: "",
        role: "admin",
        responsibility: "Admin group Perusahaan hitam | Kurogames",
        isPrimary: true
      },
      {
        platform: "discord",
        url: "https://discord.gg/users/458699026880266280",
        role: "admin",
        responsibility: "Admin group Perusahaan hitam | Kurogames",
        isPrimary: true
      }
    ]
  },

  {
    slug: "hika",
    name: "Hika",
    img: "/admins/hika.jpg",
    role: "Co-owner Group",
    gender: "perempuan",
    skills: [
      "Draw",
      "Game Guide",
      "Cosplay",
      "Leluhur Game Ryhthm"
    ],
    responsibility: "Membantu member yang kesulitan",
    joined: 2021,
    divisions: [
      "Admin Grup",
      "Gvs"
    ],
    languages: [
      "ID"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T16:45:03.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "mlbb",
      "hsr",
      "wuwa",
      "ak",
      "zzz",
      "roblox",
      "pjsk",
      "bandori",
      "d4dj",
      "rhythm-hive"
    ],
    whatsapp: "6289654234464",
    channels: [
      {
        platform: "whatsapp",
        contact: "6289654234464",
        role: "admin",
        responsibility: "Membantu member yang kesulitan",
        isPrimary: true
      }
    ]
  },

  {
    slug: "gavrill",
    name: "Hilmi",
    img: "/admins/gavrill.jpg",
    role: "Admin",
    gender: "laki laki",
    skills: [
      "Guide"
    ],
    responsibility: "Admin grup Anak Emas Paimon, leaks Genshin",
    joined: 2024,
    divisions: [
      "Leaks",
      "Admin Grup"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T16:48:08.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "mlbb",
      "genshin"
    ],
    whatsapp: "6285314370807",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285314370807",
        role: "admin",
        responsibility: "Admin grup Anak Emas Paimon, leaks Genshin",
        isPrimary: true
      }
    ]
  },

  {
    slug: "yuuma",
    name: "Yuuma",
    img: "/admins/yuuma.jpg",
    role: "Best Creator",
    gender: "laki laki",
    skills: [
      "Membuat Konten Kreatif",
      "Membuat Guide Game Gacha",
      "Membuat Reels yang Original",
      "Ahli dalam Game Strategi"
    ],
    responsibility: "Memandu Newbie Agar Tidak Tersesat di game HsR",
    joined: 2021,
    divisions: [
      "Babel",
      "Posting",
      "Admin Grup",
      "Discord",
      "Partnership"
    ],
    languages: [
      "ID",
      "EN",
      "Jawa"
    ],
    timezone: "UTC+7 (Jakarta)",
    status: "active",
    lastUpdated: "2025-10-06T16:48:44.000Z",
    platforms: [
      "whatsapp",
      "discord"
    ],
    games: [
      "genshin",
      "hsr",
      "wuwa",
      "ak",
      "ba",
      "pgr",
      "fgo",
      "gbf"
    ],
    whatsapp: "6285172268228",
    discord: "https://discord.gg/adamsan#2010",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285172268228",
        role: "admin",
        responsibility: "Memandu Newbie dan Sharing Seputar HsR",
        isPrimary: true
      },
      {
        platform: "discord",
        url: "https://discord.gg/adamsan#2010",
        role: "admin",
        responsibility: "Memandu Newbie Agar Tidak Tersesat di game HsR",
        isPrimary: true
      }
    ]
  },
    {
    slug: "yura",
    name: "Yura Naomi",
    img: "/admins/yura.jpg",
    role: "mascot",
    gender: "perempuan",
    skills: [
      "Kecerdasan Buatan",
      "Asissten Cerdas Komunitas",
      "Konsultasi Kesehatan Mental",
      "Pemegang Database Komunitas Gachaverse"
    ],
    responsibility: "AI Asisten Komunitas Gachaverse",
    joined: 2024,
    divisions: [
      "Babel",
      "Semua Divisi"
    ],
    languages: [
      "Semua Bahasa"
    ],
    timezone: "Not Realtime",
    status: "active",
    lastUpdated: "2025-10-06T16:48:44.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "#"
    ],
    whatsapp: "6285179546561",
    discord: "https://discord.gg/adamsan#2010",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285179546561",
        role: "mascot",
        responsibility: "Asisten Komunitas",
        isPrimary: true
      },
         ]
  },
      {
    slug: "aileen",
    name: "Aileen",
    img: "/admins/aileen.jpg",
    role: "Translator",
    gender: "perempuan",
    skills: [
      "Bantu translate CN dari game manapun",
      "mampu mengajar CN diluar kegiatan tugas sebagai admin translator CN"
    ],
    responsibility: "Membantu admin leak terjemahkan CN",
    joined: 2024,
    divisions: [
      "Semua Divisi"
    ],
    languages: [
      "ID",
      "EN",
      "CN"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-06T16:48:44.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "wuwa",
      "hi3",
      "roblox"
    ],
    whatsapp: "6282188884208 ",
    channels: [
      {
        platform: "whatsapp",
        contact: "6282188884208",
        role: "Translator",
        responsibility: "Penerjemah CN Komunitas",
        isPrimary: true
      },
         ]
  },
{
  slug: "chuzo",
  name: "Chuzo",
  img: "/admins/chuzo.jpg",
  role: "Co-founder",
  gender: "laki laki",
  skills: ["Psychology", "Counseling", "communication", "Checker Translation (EN)"],
  responsibility: "Menggantikan founder bila berhalangan dan membantu meringankan tugas founder.",
  joined: 2023,

  divisions: ["Babel", "Translator"],
  languages: ["ID", "EN"],
  timezone: "WIB",
  status: "active",
  lastUpdated: "2025-10-07T12:00:05.000Z",

  platforms: ["whatsapp", "discord"],
  games: ["gi", "ak", "pjsk", "bandori", "limbus", "tot"],

  discord: "https://discord.com/invite/QhSe6q7g",

  channels: [
    { platform: "whatsapp", role: "co-founder", responsibility: "Koordinasi harian komunitas.", isPrimary: true },
    { platform: "discord", url: "https://discord.com/invite/QhSe6q7g", role: "co-founder", responsibility: "Koordinasi & komunikasi tim." }
  ]
},
{
  slug: "mitsune2eima",
  name: "Mitsune2eima",
  img: "/admins/mitsune2eima.jpg",
  role: "Admin",
  gender: "laki laki",
  skills: ["Guide", "Editing", "Lore Telling"],
  responsibility: "Mengawasi alur percakapan di grup WhatsApp, menjawab pertanyaan, dan mengeksekusi pelanggaran.",
  joined: 2024,

  divisions: ["Admin Grup"],
  languages: ["ID", "EN", "Belanda", "Swedia", "Italia"],
  timezone: "WITA",
  status: "active",
  lastUpdated: "2025-10-07T12:00:10.000Z",

  platforms: ["whatsapp"],
  games: ["pgr", "hok", "fgo"],

  whatsapp: "6281210127259",

  channels: [
    { platform: "whatsapp", contact: "6281210127259", role: "admin", responsibility: "Moderasi & QnA di grup.", isPrimary: true }
  ]
},
{
  slug: "hitori",
  name: "Hitori",
  img: "/admins/hitori.jpg",
  role: "Admin",
  gender: "laki laki",
  skills: ["Translator", "Community Care"],
  responsibility: "Menjaga suasana grup tetap nyaman, sigap membantu, dan translasi JP bila diperlukan.",
  joined: 2023,

  divisions: ["Admin Grup", "Translator"],
  languages: ["ID", "JP"],
  timezone: "UTC+9",
  status: "active",
  lastUpdated: "2025-10-07T12:00:15.000Z",

  platforms: ["whatsapp"],
  games: ["genshin", "hsr", "wuwa", "ba", "hi3", "zzz", "persona"],

  channels: [
    { platform: "whatsapp", role: "admin", responsibility: "Moderasi & bantuan member.", isPrimary: true }
  ]
},
{
  slug: "kenji",
  name: "Kenji",
  img: "/admins/kenji.jpg",
  role: "Co-founder",
  gender: "laki laki",
  skills: ["Guide", "Information"],
  responsibility: "Mengawasi grup dan memberikan informasi/guide terkait game.",
  joined: 2020,

  divisions: ["Admin Grup"],
  languages: ["ID"],
  timezone: "WIB",
  status: "active",
  lastUpdated: "2025-10-07T12:00:20.000Z",

  platforms: ["whatsapp"],
  games: ["wuwa", "pgr"],

  whatsapp: "6289618586286",

  channels: [
    { platform: "whatsapp", contact: "6289618586286", role: "co-founder", responsibility: "Koordinasi & pengawasan grup.", isPrimary: true }
  ]
},
{
  slug: "chippy",
  name: "Chippy",
  img: "/admins/chippy.jpg",
  role: "Owner Group",
  gender: "laki laki",
  skills: ["Rhythm Games", "Art"],
  responsibility: "Owner FullCombo dan pengelolaan komunitas rhythm (event/konten).",
  joined: 2022,

  divisions: ["Owner Grup", "Admin Grup", "Discord"],
  languages: ["ID", "EN"],
  timezone: "WIB",
  status: "active",
  lastUpdated: "2025-10-07T12:00:25.000Z",

  platforms: ["whatsapp", "discord"],
  games: ["mlbb", "wuwa", "pjsk", "uma"],

  whatsapp: "6287888350801",
  discord: "https://discord.gg/butterchipstm",

  channels: [
    { platform: "whatsapp", contact: "6287888350801", role: "owner", responsibility: "Koordinasi & keputusan utama komunitas.", isPrimary: true },
    { platform: "discord", url: "https://discord.gg/butterchipstm", role: "owner", responsibility: "Event & komunikasi server." }
  ]
},
{
  slug: "ren",
  name: "Ren",
  img: "/admins/ren.jpg",
  role: "Co-Owner",
  gender: "laki laki",
  skills: ["Video Editing", "Event Organizer", "Moderation"],
  responsibility: "Menjaga ketertiban anggota ACM dan membuat mini event untuk grup.",
  joined: 2022,

  divisions: ["Admin Grup"],
  languages: ["EN", "JP","ID"],
  timezone: "WIB",
  status: "active",
  lastUpdated: "2025-10-07T12:00:30.000Z",

  platforms: ["whatsapp"],
  games: ["mlbb", "hsr", "wuwa", "ba"],

  whatsapp: "6281381340997",

  channels: [
    {
      platform: "whatsapp",
      contact: "6281381340997",
      role: "Co-Owner",
      responsibility: "Menjaga ketertiban anggota ACM & menjalankan mini event.",
      isPrimary: true
    }
  ]
},
{
    slug: "cid",
    name: "Cid",
    img: "/admins/cid.jpg",
    role: "Co-owner Group",
    gender: "laki-laki",
    skills: [
      "Guide game HSR"
    ],
    responsibility: "Mengawasi grup yang dipegang, serta memberikan panduan seputar HSR",
    joined: 2023,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-08T00:00:00.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "hsr"
    ],
    whatsapp: "6285814060508",
    discord: "",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285814060508",
        role: "admin",
        responsibility: "Guide seputar HSR",
        isPrimary: true
      }
    ]
  },
  {
    slug: "damar",
    name: "Marz",
    img: "/admins/damar.jpg",
    role: "admin",
    gender: "laki-laki",
    skills: [
      "Guide game",
      "Moba",
      "Video Editing"
    ],
    responsibility: "Sebagai salah satu admin ACM sebisa mungkin membantu grup dan member.",
    joined: 2023,
    divisions: [
      "Admin Grup"
    ],
    languages: [
      "ID"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-08T00:00:00.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "mlbb",
      "hsr",
      "minecraft"
    ],
    whatsapp: "6285878234525",
    discord: "",
    channels: [
      {
        platform: "whatsapp",
        contact: "6285878234525",
        role: "admin",
        responsibility: "",
        isPrimary: true
      }
    ]
  },
  {
    slug: "lepus-0810",
    name: "Na0",
    img: "/admins/na0.jpg",
    role: "moderator",
    gender: "laki-laki",
    skills: [
      "Guide",
      "Moderating",
      "Penggunaan bot"
    ],
    responsibility: "Berperan sebagai moderator di grup discord, menjaga keamanan grup",
    joined: 2025,
    divisions: [
      "Discord"
    ],
    languages: [
      "ID"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-08T00:00:00.000Z",
    platforms: [
      "whatsapp",
      "discord"
    ],
    games: [
      "genshin",
      "persona",
      "NFS"
    ],
    whatsapp: "81256792690",
    discord: "https://discord.com/users/1240322788171059303",
    channels: [
      {
        platform: "whatsapp",
        contact: "81256792690",
        role: "admin",
        responsibility: "",
        isPrimary: false
      },
      {
        platform: "discord",
        url: "https://discord.com/users/1240322788171059303",
        role: "moderator",
        responsibility: "Moderator",
        isPrimary: true
      }
    ]
  },
  {
    slug: "the-real-kanee",
    name: "Ludfi",
    img: "/admins/ludfi.jpg",
    role: "admin",
    gender: "laki-laki",
    skills: [
      "Marketing",
      "Transaction Management",
      "Networking"
    ],
    responsibility: "Mengelola transaksi akun game",
    joined: 2024,
    divisions: [
      "Discord"
    ],
    languages: [
      "ID"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-08T00:00:00.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "mlbb",
      "genshin",
      "hsr",
      "wuwa",
      "Free fire"
    ],
    whatsapp: "6282125793171",
    discord: "",
    channels: [
      {
        platform: "whatsapp",
        contact: "6282125793171",
        role: "admin",
        responsibility: "Mengelola transaksi akun game",
        isPrimary: false
      }
    ]
  },
  {
    slug: "al-schariac",
    name: "AL",
    img: "/admins/al.jpg",
    role: "moderator",
    gender: "laki-laki",
    skills: [
      "Dad jokes",
      "Story telling"
    ],
    responsibility: "Menjaga keamanan discord",
    joined: 2025,
    divisions: [
      "Discord"
    ],
    languages: [
      "ID"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-08T00:00:00.000Z",
    platforms: [
      "whatsapp",
      "discord"
    ],
    games: [
      "genshin",
      "hsr",
      "wuwa",
      "zzz",
      "roblox",
      "Path to Nowhere"
    ],
    whatsapp: "6281384322093",
    discord: "https://discordapp.com/al_schariac/69685309643",
    channels: [
      {
        platform: "whatsapp",
        contact: "6281384322093",
        role: "",
        responsibility: "",
        isPrimary: false
      },
      {
        platform: "discord",
        url: "https://discordapp.com/al_schariac/69685309643",
        role: "moderator",
        responsibility: "Keamanan",
        isPrimary: true
      }
    ]
  },
  {
    slug: "rein",
    name: "Reinavy",
    img: "/admins/rein.jpg",
    role: "admin",
    gender: "laki-laki",
    skills: [
      "Graphic Design(slightly)",
      "Motion Design"
    ],
    responsibility: "Upload postingan instagram sesuai keahlian sendiri.",
    joined: 2025,
    divisions: [
      "Posting"
    ],
    languages: [
      "ID",
      "EN"
    ],
    timezone: "WITA",
    status: "active",
    lastUpdated: "2025-10-08T00:00:00.000Z",
    platforms: [
      "whatsapp",
      "discord"
    ],
    games: [
      "wuwa"
    ],
    whatsapp: "6282154749208",
    discord: "https://discord.gg/853225956603723808",
    channels: [
      {
        platform: "whatsapp",
        contact: "6282154749208",
        role: "admin",
        responsibility: "Upload postingan instagram sesuai keahlian sendiri.",
        isPrimary: true
      },
      {
        platform: "discord",
        url: "https://discord.gg/853225956603723808",
        role: "moderator",
        responsibility: "Discord nya gua gaada role..",
        isPrimary: true
      }
    ]
  },
  {
    slug: "yurian",
    name: "Yurian",
    img: "/admins/yurian.jpg",
    role: "Best Creator",
    gender: "perempuan",
    skills: [
      "Writing"
    ],
    responsibility: "Menulis lore dan informasi dari Story Game dengan cara yang mudah dipahami pembaca.",
    joined: 2025,
    divisions: [
      "Posting"
    ],
    languages: [
      "ID",
      "EN",
      "JP"
    ],
    timezone: "WIB",
    status: "active",
    lastUpdated: "2025-10-08T00:00:00.000Z",
    platforms: [
      "whatsapp"
    ],
    games: [
      "genshin",
      "hsr",
      "hi3",
      "roblox"
    ],
    whatsapp: "6288233869471",
    discord: "",
    channels: [
      {
        platform: "whatsapp",
        contact: "6288233869471",
        role: "admin",
        responsibility: "Menulis lore dan informasi dari Story Game dengan cara yang mudah dipahami pembaca.",
        isPrimary: true
      }
    ]
  }
];

export function getAdminBySlug(slug) {
  return admins.find(a => a.slug === slug);
}
